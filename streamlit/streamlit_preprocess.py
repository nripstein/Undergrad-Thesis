...
# def image_bounds_set_size_method_og(min_max_tuple: tuple, size_x: int = 652, size_y: int = 652) -> tuple[
...
def image_bounds_set_size_within_frame(min_max_tuple: tuple, size_x: int = 652, size_y: int = 652, frame_size: tuple = (1920, 1080)) -> tuple:
    """
    Modified function to ensure the bounding box is within the frame size. 

    Args:
        min_max_tuple (tuple[tuple[int, int], tuple[int, int]]): bounding box around hand coordinates of form ((x_min, x_max), (y_min, y_max)).
        size_x (int): x dimension of resulting bounding box.
        size_y (int): y dimension of resulting bounding box.
        frame_size (tuple[int, int]): the size of the frame as (width, height).
    ...
    # Calculate the center of the hands bounding box
    delta_x = x[1] - x[0]
    delta_y = y[1] - y[0]
    x_center = x[0] + delta_x // 2
    y_center = y[0] + delta_y // 2

    # Calculate the new bounding box, ensuring it stays within the frame
    half_size_x = size_x // 2
    half_size_y = size_y // 2

    x_min = max(x_center - half_size_x, 0)
    x_max = min(x_center + half_size_x, frame_width)

    y_min = max(y_center - half_size_y, 0)
    y_max = min(y_center + half_size_y, frame_height)

    # Adjust if the bounding box is larger than the frame
    if x_max - x_min < size_x:
        diff = size_x - (x_max - x_min)
        x_min = max(x_min - diff, 0)
        x_max = min(x_max + diff, frame_width)

    if y_max - y_min < size_y:
        diff = size_y - (y_max - y_min)
        y_min = max(y_min - diff, 0)
        y_max = min(y_max + diff, frame_height)
...
if uploaded_video is not None:
    ...
    img_height, img_width, channels = processed_image.shape
    st.success(f"{img_width, img_height}")
    ...
    if detection_result.multi_hand_landmarks is None:  # if no hands are detected, display normally
        ...
        st.warning("No hands detected! (manual crop required)")
        st.stop()

    # bounds_sized = image_bounds_set_size_og(output_tuple)
    bounds_sized = image_bounds_set_size_within_frame(output_tuple, frame_size=(img_width, img_height))
